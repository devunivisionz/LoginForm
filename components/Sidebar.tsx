  ...
  return (
    <aside className="w-full max-w-[100%]  lg:max-w-96  bg-[#E8F0FD] p-4 sm:p-6 !pl-[14px] !pt-[14px] lg:p-8 flex flex-col gap-0   lg:min-h-screen">
      <div className="flex-0 sm:flex-1 space-y-6">
        ...
        {/* Headline */}
        <div>
          <h2 className="text-2xl sm:text-3xl lg:text-[32px] leading-12 tracking-[-1px] font-bold text-gray-900">
            Your <span className="text-blue-600">Work Note</span> is Minutes Away
          </h2>
          <p className="text-sm sm:text-base text-[#111827] mt-[21px]">
      ...
      {/* Carousel */}
      <div className="relative overflow-hidden w-full mt-[3rem] sm:mt-6">
        <div
      ...
      {/* Dots */}
      <div className="flex space-x-2 pt-4   max-sm:justify-center justify-left">
        {testimonials.map((_, index) => (
          <div
            key={index}
            onClick={() => setCurrentIndex(index)}
            className={`w-[10px] h-[10px] sm:w-3 sm:h-3 rounded-full cursor-pointer transition-colors ${currentIndex === index ? "bg-blue-600" : "bg-blue-200"
              }`}
          />
        ))}
      </div>
